...
import {
  ...
  CheckCircle as CheckIcon, Loader2, Users, Activity,
  Send, ExternalLink, Copy, AlertTriangle,
} from 'lucide-react';
...
const translations: any = {
  tr: {
    ...
    hero: {
      ...
      // FIX #7 — Daha açıklayıcı uyarı metni
      autoRedirectWarn: "⚠️ Bypass tamamlandığında bu sekme otomatik kapanacak!",
      paste: "Yapıştır",
      // FIX #3 — Protocol ekleme bildirimi
      protocolAdded: "https:// otomatik eklendi",
      invalidUrl: "Geçerli bir URL girin (örn: ouo.io/xxx)",
      clear: "Temizle",
      newBypass: "Yeni Link",
    },
    safety: {
    ...
    queue: {
      ...
      // FIX #8 — Polling süre göstergesi
      elapsedPrefix: "Geçen süre:",
    },
  ...
  en: {
    ...
    hero: {
      ...
      autoRedirectWarn: "⚠️ This tab will automatically redirect when bypass is complete!",
      paste: "Paste",
      protocolAdded: "https:// added automatically",
      invalidUrl: "Enter a valid URL (e.g. ouo.io/xxx)",
      clear: "Clear",
      newBypass: "New Link",
    },
    safety: {
...
const LanguageSelector = ({ currentLang, setLang, isDarkMode }: any) => {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {isOpen && (
        // FIX #4 — z-index: dil dropdown'ı her zaman en üstte (reklam panellerinin üstünde)
        <div className={`absolute top-full right-0 mt-2 w-44 rounded-xl border shadow-2xl overflow-hidden z-[100]
...
export default function Home() {
  ...
  // FIX #1 — Dark mode SSR flash önleme
  // İlk render'da undefined (SSR), client'ta localStorage/sistem tercihinden okunur.
  // Bu şekilde hydration mismatch ve flash olmaz.
  const [isDarkMode, setIsDarkMode] = useState<boolean | undefined>(undefined);

  useEffect(() => {
    // layout.tsx'deki inline script body'ye 'light-mode' eklediyse light, yoksa dark
    const isLight = document.body.classList.contains('light-mode');
    setIsDarkMode(!isLight);
  }, []);

  // ── Tüm hook'lar erken return'den ÖNCE tanımlanmalı (Rules of Hooks) ──
  ...
  // FIX #8 — Polling süresi
  const [elapsedSeconds, setElapsedSeconds] = useState(0);
  // FIX #3 — Protocol eklendi bildirimi
  const [protocolAdded, setProtocolAdded] = useState(false);

  // Sonuç kutusuna scroll için ref
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setStats(prev => ({
        ...
        users: Math.max(1, prev.users + Math.floor(Math.random() * 4) - 1),
      }));
    }, 4000);
    return () => clearInterval(interval);
  }, []);

  // FIX #8 — Yükleme süresi sayacı
  useEffect(() => {
  ...
  // FIX #1 — Hydration tamamlanana kadar render etme (hook'lardan SONRA)
  ...
  // FIX #3 — Protocol normalizasyonu
  // Kullanıcı "ouo.io/abc" yazdığında otomatik "https://" ekler.
  ...
  const handleUrlChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Sadece boşluksuz bir şey yazılıp focus kaybedildiğinde normalize et.
    // Yazarken normalize etmek cursor pozisyonunu bozar, bu yüzden sadece
    // kullanıcı yapıştırırken veya submit ederken normalize ediyoruz.
    setUrl(raw);
  };
  ...
  // URL doğrulama — FIX #3: artık daha geniş pattern
  ...
  const handleBypass = async () => {
    ...
    if (!finalUrl || !isValidUrl(finalUrl)) return;

    // URL normalize edildiyse state'i güncelle
    if (finalUrl !== url) setUrl(finalUrl);

    setIsLoading(true);
    // FIX #2 — Hata durumunda sadece error temizlenir, URL korunur
    setError('');
    setResultLink('');
        ...
        if (data.queue_position != null) {
          ...
          setStatusMessage(
            data.queue_position === 0
              ? t.queue.processing
              : `${t.queue.inQueue} ${data.queue_position}. ${t.queue.position}...`
          );
        }
        if (data.status === 'success') {
          clearInterval(interval);
          ...
          setQueuePosition(null);
          setError(
            data.fail_reason === 'link_not_found' ? t.queue.notFound
              : data.fail_reason === 'timeout' ? t.queue.timeout
                : t.queue.retryFailed
          );
          setIsLoading(false);
        }
  ...
  const finishProcess = (finalUrl: string, linkId?: number, initialSafety?: string) => {
    ...
    // FIX #10 — Mobil: sonuç görünür hale gelince scroll et
    setTimeout(() => {
  ...
  // FIX #9 — Yeni bypass başlatmak için state sıfırla (URL'yi silme, sonucu sil)
  const handleNewBypass = () => {
    setResultLink('');
    setError('');
    setSafetyStatus(null);
    setBypassId(null);
  ...
  return (
    <main className={`min-h-screen flex flex-col relative overflow-x-hidden transition-colors duration-300
      ...
      {/* ══════════ HEADER ══════════ */}
      {/* FIX #4 — Header z-index artırıldı, dropdown için alan bırakıldı */}
      <header className={`fixed top-0 w-full h-16 border-b z-[80] backdrop-blur-xl transition-colors
        ...
        <div className="max-w-6xl mx-auto h-full flex items-center justify-between px-4 md:px-8">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* FIX #1 — toggleDarkMode artık localStorage'a da yazıyor */}
            <button onClick={toggleDarkMode} className="p-2 rounded-lg hover:bg-white/5 transition-colors">
              {isDarkMode
      ...
      {/*
        FIX #5 — AdUnit hizalama
        Sol/sağ paneller top-[114px] = header(64px) + banner(50px).
        İçerik alanı xl:mx-[160px] ile her section'da tutarlı şekilde kaydırılıyor,
        sadece hero'da değil.
      */}
      <div className={`hidden xl:flex fixed left-0 top-[114px] bottom-0 w-[160px] border-r overflow-hidden z-[10]
      ...
      <div className={`hidden xl:flex fixed right-0 top-[114px] bottom-0 w-[160px] border-l overflow-hidden z-[10]
        ${isDarkMode ? 'bg-black/80 border-white/5' : 'bg-gray-100 border-gray-300'}`}>
        <AdUnit client="ca-pub-XXXXXXXXXX" slot="XXXXXXX" format="auto" />
      </div>

      {/* FIX #5 — Tüm içerik xl:mx-[160px] ile tutarlı hizalandı */}
      <div className="xl:mx-[160px]">
        ...
        <section className="relative pt-10 md:pt-16 pb-8 px-4">
          ...
          <div className="max-w-3xl mx-auto text-center relative z-10">
            ...
            <div className={`max-w-2xl mx-auto rounded-2xl p-1.5 transition-all
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                ...
                <button
                  ...
                  disabled={isLoading || !url || !isValidUrl(normalizeUrl(url))}
            ...
            {/* FIX #3 — Protocol eklendi bildirimi */}
            {protocolAdded && (
              <p className="text-xs text-purple-400 text-center mt-2 animate-in">
                ✓ {t.hero.protocolAdded}
              </p>
            )}

            {/* URL uyarısı — FIX #3: daha geniş validation ile daha az false positive */}
            ...
            <div className="flex flex-col items-center gap-2 mt-4">
              ...
              {/* FIX #7 — Auto-redirect uyarısı daha belirgin */}
              {autoRedirect && (
            ...
            <div className="max-w-2xl mx-auto mt-6 space-y-4">
              ...
              {/* FIX #2 — Hata: URL silinmiyor, sadece error temizlenir */}
              {error && (
                <div className={`p-4 rounded-2xl text-center font-medium text-sm animate-in flex flex-col items-center gap-3
              ...
              {/* FIX #8 — Yükleniyor: geçen süre gösteriliyor */}
              {isLoading && !error && (
                <div className="text-center space-y-3">
                  ...
                  {elapsedSeconds > 0 && (
                    <p className={`text-xs ${isDarkMode ? 'text-gray-600' : 'text-gray-400'}`}>
                      {t.queue.elapsedPrefix} {elapsedSeconds}s
                    </p>
                  )}
              ...
              {/* Başarılı Sonuç */}
              {/* FIX #10 — resultRef ile scroll hedefi */}
              {resultLink && (
                <div
                  ref={resultRef}
                  className={`p-5 rounded-2xl border space-y-4 animate-in zoom-in-95 duration-300
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    {/* FIX #9 — Yeni bypass butonu: URL'yi silmez, sonucu temizler */}
                    <button
                      onClick={handleNewBypass}
                  ...
                  <div className={`flex items-center gap-2 p-3 rounded-xl ${isDarkMode ? 'bg-black/30' : 'bg-white border'}`}>
                    <input
                      ...
                      className="bg-transparent w-full outline-none text-sm text-gray-400 cursor-text select-all"
                    />
                    <button
