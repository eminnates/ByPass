...
from app.constants import SafetyStatus
...
def parse_vt_stats(stats):
    if not stats: return SafetyStatus.UNKNOWN
    if stats.get("malicious", 0) > 0: return SafetyStatus.MALICIOUS
    elif stats.get("suspicious", 0) > 0: return SafetyStatus.SUSPICIOUS
